...
func (r *PostgresDB) UpdateUser(ctx context.Context, id string, p *model.UserModel) error {
	a, err := r.Pool.Exec(ctx, "update persons set username=$1,name=$2 where id=$4", &p.Name, &p.Name, id)
	if a.RowsAffected() == 0 {
		return fmt.Errorf("user with this id doesnt exist")
	}
	if err != nil {
